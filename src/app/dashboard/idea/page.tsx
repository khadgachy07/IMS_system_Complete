...
export default function Ideas() {
  ...
  const handleIncentive = async (user: User, ideaId: number) => {
    console.log(user);
    const token = getAuthToken();
    if (!token) {
    ...
    try {
      const response = await fetch(`/api/incentive`, {
        ...
        body: JSON.stringify({ userId: user.id, ideaId: ideaId }),
      });
        ...
        >
          <div className="bg-teal-800 rounded-lg shadow-lg p-6 w-full max-w-md">
            ...
            {userRole === Role.SYSTEM_ADMIN && selectedIdea?.submittedBy && (
              <button
                onClick={() => handleIncentive(selectedIdea.submittedBy, selectedIdea.id)}
