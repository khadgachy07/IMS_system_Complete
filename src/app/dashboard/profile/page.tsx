    ...
    const fetchUserProfileAndData = async () => {
      ...
      try {
        ...
        if (user.user.role === "employee") {
          ...
          const submittedIdeas: Idea[] = ideaData
            .filter((idea) => idea?.submittedBy?.id === user.user.userId)
            .sort((a, b) => {
              // Ensure submittedAt is in valid date format (if it's a string, parse it)
              return (
