...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: number }> }) {
  try {
    ...
    if (idea.submittedBy.id === userEntity.id) {
      return new NextResponse("You cannot vote for your own idea", {
        status: 400,
      });
    }

    // Prevent multiple votes
    const alreadyVoted = idea.votedBy.some(
      (voter) => voter.id === userEntity.id,
    );
    if (alreadyVoted) {
      return new NextResponse("You have already voted for this idea", {
        status: 400,
      });
    }
